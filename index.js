...
app.use("/user", authToken, authRole("user"), (req, res) => {
  ...
  proxy.web(req, res, {
    target: "http://13.218.238.30:5001",
    headers: {
...
app.use("/admin", authToken, authRole("admin"), (req, res) => {
  ...
  proxy.web(req, res, { target: "http://13.218.238.30:5002" });
});

app.use("/blog", (req, res) => {
  console.log("INSIDE API GATEWAY BLOG ROUTE");
  proxy.web(req, res, { target: "http://3.83.112.120:5003" }); // or whatever port you use
});
...
app.use("/auth", (req, res) => {
  ...
  proxy.web(req, res, { target: "http://54.89.139.190:5000" });
});
